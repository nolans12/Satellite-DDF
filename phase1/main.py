...
# Define a sensor model:
    sens1 = sensor(fov = 115, sensorError = np.array([0, 0]), detectError= 0.05, resolution = 720, name = 'Sensor 1')
    sens2 = sensor(fov = 100, sensorError = np.array([2, 2]), detectError= 0.05, resolution = 720, name = 'Sensor 2')
    
# Define targets for the satellites to track:
    targetIDs1 = np.array([1])
    targetIDs2 = np.array([1])
...
# Define some polar orbits at 1000 km altitude:
    sat1 = satellite(name = 'Sat1', sensor = sens1, targetIDs=targetIDs1, estimator = local1, a = Earth.R + 1000 * u.km, ecc = 0, inc = 90, raan = 0, argp = 50, nu = 0, color='b')
    sat2 = satellite(name = 'Sat2', sensor = sens2, targetIDs=targetIDs2, estimator = local2, a = Earth.R + 1000 * u.km, ecc = 0, inc = 90, raan = 0, argp = 40, nu = 0, color='r')
...
# Define some targets:
    ...
    #targ3 = target(name = 'Targ3', targetID=3, r = np.array([6378, 0, np.deg2rad(40), 0, 0, 0]),color = 'g')
    #targ4 = target(name = 'Targ4', targetID=4, r = np.array([6378, 0, np.deg2rad(60), 0, 0, 0]),color = 'y')
