...
def plot_NEES_NIS(simData):
    ...
    for targ in nees_net.keys():
        ...
        for sat in nees_net[targ].keys():
            ...
            nees_avg = np.array(
                [
                    np.mean(nees_net[targ][sat][time])
                    for time in nees_net[targ][sat].keys()
                ]
            )
            nis_avg = np.array(
                [
                    np.mean(nis_net[targ][sat][time])
                    for time in nis_net[targ][sat].keys()
                ]
            )
            # Plot the data
