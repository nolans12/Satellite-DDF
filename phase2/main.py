...
def create_environment():
    ...
    # We will use the RdPu color map for the targets
    RdPu = ['#FEEBE2', '#FCC5C0', '#FA9FB5', '#F768A1', '#DD3497', '#AE017E', '#7A0177', '#49006A', '#49006A', '#49006A', '#49006A', '#49006A']

    # Define a random target:
    targ1 = target(name = 'Targ1', targetID=1, coords = np.array([0,0,0]), heading=0, speed= 0, color = RdPu[0])

    targs = [targ1]

    # Define the satellite structure:

    sens = sensor(name = 'Sensor', fov = 115, bearingsError = np.array([115 * 0.1, 115 * 0.1])) # 1% error on FOV bearings
    ...
    green_shades = ['#32CD32', '#556B2F']
    yellow_shades = ['#FDDA0D', '#FFA500']

    # Define the satellites:
    sat1 = satellite(name = 'Sat1a', sensor = deepcopy(sens), targetIDs=targetIDs, indeptEstimator=deepcopy(local), ddfEstimator=deepcopy(ddf), a = Earth.R + 1000 * u.km, ecc = 0, inc = 90, raan = -45, argp = 0, nu = 0, color=green_shades[0])
    sat2 = satellite(name = 'Sat1b', sensor = deepcopy(sens), targetIDs=targetIDs, indeptEstimator=deepcopy(local), ddfEstimator=deepcopy(ddf), a = Earth.R + 1000 * u.km, ecc = 0, inc = 90, raan = -45, argp = -25, nu = 0, color=green_shades[1])
    sat3 = satellite(name = 'Sat2a', sensor = deepcopy(sens), targetIDs=targetIDs, indeptEstimator=deepcopy(local), ddfEstimator=deepcopy(ddf), a = Earth.R + 1000 * u.km, ecc = 0, inc = 90, raan = -45, argp = -50, nu = 0, color=yellow_shades[0])
    sat4 = satellite(name = 'Sat2b', sensor = deepcopy(sens), targetIDs=targetIDs, indeptEstimator=deepcopy(local), ddfEstimator=deepcopy(ddf), a = Earth.R + 1000 * u.km, ecc = 0, inc = 0, raan = -45, argp = 0, nu = 0, color=yellow_shades[1])
...
if __name__ == "__main__":
    ...
    # print('Starting NEES and NIS test')

    # time_vec = np.linspace(0, 20, 20 + 1) * u.minute
    # # time_vec = np.linspace(40, 50, 10 + 1) * u.minute
    # fileName = "testEnv" 
    # numSims = 1
    ...
    #     # Simulate the satellites through the vector of time:
    #     simData[i] = env.simulate(time_vec, savePlot = True, showSim = True, saveName = fileName)
    #     env.render_gif(fileType='satellite_simulation', saveName=fileName, fps = 5)
...
#         env = create_environment_edge()
#         # Simulate the satellites through the vector of time:
#         simData[i] = env.simulate(time_vec, showSim=True, saveGif=True, saveData= True, savePlot = True, saveName = fileName)
#         env.render_gif(fileType='satellite_simulation', saveName=fileName, fps = 5)
#         # simData[i] = env.simulate(time_vec)
