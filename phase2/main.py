...
def create_environmen_change_priorities():
    ...
    targ1 = target(name = 'Targ1', tqReq = 1, targetID=1, coords = np.array([45,0,0]), heading=0, speed= 80,  uncertainty=np.array([3, 7.5, 0, 90, 0.1]), color = 'darkred')
    targ2 = target(name = 'Targ2', tqReq = 2, targetID=2, coords = np.array([45,0,0]), heading=0, speed= 50, uncertainty=np.array([3, 7.5, 0, 90, 0.1]), color = 'slategrey')
    targ3 = target(name = 'Targ3', tqReq = 3, targetID=3, coords = np.array([45,0,0]), heading=0, speed= 40,  uncertainty=np.array([3, 7.5, 0, 90, 0.1]), color = 'peru')
    targ4 = target(name = 'Targ4', tqReq = 4, targetID=4, coords = np.array([45,0,0]), heading=0, speed= 30,  uncertainty=np.array([3, 7.5, 0, 90, 0.1]), color = 'darkorange')
    targ5 = target(name = 'Targ5', tqReq = 5, targetID=5, coords = np.array([45,0,0]), heading=0, speed= 20,  uncertainty=np.array([3, 7.5, 0, 90, 0.1]), color = 'dodgerblue')
    ...
    # For each satellite define a dictionary that defines which targetIDs they should track and to what percision they should track them too!

    #     # Contains the targetID and the priority of tracking that target
    # targPriority1 = {1: 100, 2: 150, 3: 200, 4: 250, 5: 300}

    # targPriority2 = {1: 300, 2: 300, 3: 300, 4: 300, 5: 100}

    # Define the satellites:
    sat1a = satellite(name = 'Sat1a', sensor = deepcopy(sens2), targPriority = deepcopy(targPriority1), indeptEstimator=indeptEstimator(targPriority1), ciEstimator=ciEstimator(targPriority1), a = Earth.R + 1000 * u.km, ecc = 0, inc = 60, raan = -45, argp = 45, nu = 0, color='#669900')
    sat1b = satellite(name = 'Sat1b', sensor = deepcopy(sens2), targPriority = deepcopy(targPriority1), indeptEstimator=indeptEstimator(targPriority1), ciEstimator=ciEstimator(targPriority1), a = Earth.R + 1000 * u.km, ecc = 0, inc = 60, raan = -45, argp = 30, nu = 0, color='#66a3ff')
    sat2a = satellite(name = 'Sat2a', sensor = deepcopy(sens4), targPriority = deepcopy(targPriority2), indeptEstimator=indeptEstimator(targPriority2), ciEstimator=ciEstimator(targPriority2), a = Earth.R + 1000 * u.km, ecc = 0, inc = 120, raan = 45, argp = 45 + 7, nu = 0, color='#9966ff')
    sat2b = satellite(name = 'Sat2b', sensor = deepcopy(sens4), targPriority = deepcopy(targPriority2), indeptEstimator=indeptEstimator(targPriority2), ciEstimator=ciEstimator(targPriority2), a = Earth.R + 1000 * u.km, ecc = 0, inc = 120, raan = 45, argp = 30 + 7, nu = 0, color='#ffff33')
    ...
    # DEFINE THE COMMANDERS INTENT:
        # Vector from time step 0 - X where the priorities should be this vs time step X - Y where the priorities should be that
    ...
    # For minute 5+: We want the following tracks:
    commandersIntent[5] = {sat1a: {1: 100, 2: 150, 3: 200, 4: 250, 5: 300}, 
                           sat1b: {1: 100, 2: 150, 3: 200, 4: 250, 5: 300},
                           sat2a: {1: 300, 2: 300, 3: 300, 4: 300, 5: 150}, 
                           sat2b: {1: 300, 2: 300, 3: 300, 4: 300, 5: 150}}


    # Create and return an environment instance:
    return environment(sats, targs, comms_network, commandersIntent = commandersIntent, ciEstimator = True)
...
def create_environment():
    ...
    # For each satellite define a dictionary that defines which targetIDs they should track and to what percision they should track them too!
        ...
        # Contains the targetID and the priority of tracking that target
    targPriority = {1: 100, 2: 150, 3: 200, 4: 250, 5: 300}

    # Define the satellites:
    sat1a = satellite(name = 'Sat1a', sensor = deepcopy(sens2), targPriority = deepcopy(targPriority), indeptEstimator=deepcopy(local), ciEstimator=deepcopy(ddf), a = Earth.R + 1000 * u.km, ecc = 0, inc = 60, raan = -45, argp = 45, nu = 0, color='#669900')
    sat1b = satellite(name = 'Sat1b', sensor = deepcopy(sens2), targPriority = deepcopy(targPriority), indeptEstimator=deepcopy(local), ciEstimator=deepcopy(ddf), a = Earth.R + 1000 * u.km, ecc = 0, inc = 60, raan = -45, argp = 30, nu = 0, color='#66a3ff')
    sat2a = satellite(name = 'Sat2a', sensor = deepcopy(sens4), targPriority = deepcopy(targPriority), indeptEstimator=deepcopy(local), ciEstimator=deepcopy(ddf), a = Earth.R + 1000 * u.km, ecc = 0, inc = 120, raan = 45, argp = 45 + 7, nu = 0, color='#9966ff')
    sat2b = satellite(name = 'Sat2b', sensor = deepcopy(sens4), targPriority = deepcopy(targPriority), indeptEstimator=deepcopy(local), ciEstimator=deepcopy(ddf), a = Earth.R + 1000 * u.km, ecc = 0, inc = 120, raan = 45, argp = 30 + 7, nu = 0, color='#ffff33')
...
def create_environment_mono():
    ...
    sat1 = satellite(name = 'Sat1', sensor = deepcopy(sens), targetIDs=targetIDs, indeptEstimator=deepcopy(local), ciEstimator=deepcopy(ddf), etEstimator=deepcopy(et),a = Earth.R + 12000 * u.km, ecc = 0, inc = 0, raan = -45, argp = 0, nu = 0, color=purple_shades[0])

    # POLAR ORBIT SATELLITE
    sat2 = satellite(name = 'Sat2', sensor = deepcopy(sens), targetIDs=targetIDs, indeptEstimator=deepcopy(local), ciEstimator=deepcopy(ddf), etEstimator=deepcopy(et), a = Earth.R + 12000 * u.km, ecc = 0, inc = 90, raan = 0, argp = 0, nu = -45, color=blue_shades[0])

    # INCLINATION 50 SATELLITE
    sat3 = satellite(name = 'Sat3', sensor = deepcopy(sens), targetIDs=targetIDs, indeptEstimator=deepcopy(local), ciEstimator=deepcopy(ddf), etEstimator=deepcopy(et), a = Earth.R + 12000 * u.km, ecc = 0, inc = 50, raan = -135, argp = 0, nu = 90, color=yellow_shades[0])
...
if __name__ == "__main__":
    ...
    env = create_environmen_change_priorities()
