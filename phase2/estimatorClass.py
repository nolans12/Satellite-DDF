...
class BaseEstimator:
    ...
    def save_current_estimation_data(
        self, targetID, time, est, cov, innovation, innovationCov
    ):
        """
...
class etEstimator(BaseEstimator):
    ...
    def process_new_measurements(self, sat, envTime, comms):
        ...
        for targetID in commNode['received_measurements'][
            ...
            for i in range(
                ...
                comms.used_comm_et_data_values[targetID][sat.name][sender.name][
                    time_sent
                ] = np.array([alpha, beta])
                comms.used_comm_et_data[targetID][sat.name][sender.name][
                    time_sent
                ] = measVec_size
    ...
    def explicit_measurement_update(
        self, sat, sender, measurement, type, targetID, envTime, filter=None
    ):
        '''
    ...
    def synchronize_filters(self, sat, neighbor, targetID, envTime, comms):
        ...
        comms.used_comm_et_data[targetID][sat.name][neighbor.name][envTime] = (
            6 + 21
        )  # six for estimate and upper triangular of 6x6 covariance matrix
