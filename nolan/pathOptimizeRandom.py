...
for edge in g.edges():
    ...
    for targetID in track_uncertainty[source].keys():
        ...
        goodness_dict[(source, receiver, targetID)] = goodness(
            source, receiver, track_uncertainty, targetID
        )
...
path_selection_vars = pulp.LpVariable.dicts(
    ...
    [
        ...
        for targetID in track_uncertainty[path[0]].keys()
    ],
    0,
    1,
    pulp.LpBinary,
...
for path in all_paths:  # Loop through all paths possible
    ...
    for targetID in track_uncertainty[
        path[0]
...
for edge in g.edges():  # Loop through all edges possible
    ...
    prob += (
        total_bandwidth_usage <= g[u][v]['bandwidth'],
...
total_bandwidth = sum(
    value for value in g[u][v]['bandwidth'].values() for u, v in g.edges()
)
