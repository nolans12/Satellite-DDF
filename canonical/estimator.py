...
class BaseEstimator:
    def __init__(self, target_intents: dict[int, int]):
        """
        ...
        """
        self.target_intents = target_intents

        # Define the targets to track
        self.targs = target_intents.keys()

        # Define history vectors for each extended Kalman filter
        self.estHist = {
            targetID: defaultdict(dict) for targetID in target_intents.keys()
    ...
    def _initialize(self, target: target.Target, envTime: float) -> None:
        prior_pos = np.array(
    ...
    def predict(self, targetID: int, envTime: float) -> None:
        if not self.estHist[targetID]:
            self._initialize(targetID, envTime)
            return
